...
| M1 | Data augmentation | Random crop, horizontal flip, color jitter, and rotation on training only | Implemented |
| M1 | Baseline and serialized model | Five-block scratch CNN; `.pt` checkpoint and deployable `.onnx` artifact | Implemented; no pretrained weights or transfer learning |
...
| M2 | Beginner browser UX | Responsive PawSight upload/preview/results page at `/` | Implemented and included in smoke test |
| M2 | Pinned dependencies | `requirements.txt`, `requirements-prod.txt`; DVC compatibility pin | Implemented and clean-installed |
| M2 | Container | Non-root `Dockerfile`, ONNX Runtime inference, health check, production-only dependencies | Implemented |
| M2 | Local image/prediction proof | Production ONNX container plus smoke script | Passed locally; container health reached `healthy` |
...
| M5 | Real/simulated requests and labels | `/feedback`, `evaluate_deployed.py`, CSV and accuracy JSON | Passed locally: 20/20 labeled real-dataset requests correct |
